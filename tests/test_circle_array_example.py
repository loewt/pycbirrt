...
def test_example_selects_the_right_task_space(tmp_path, arm_count, expected):
    ...
    system, _path = build_system(arm_count, radius=0.9, out_dir=tmp_path)
...
def test_dilation_slider_resizes_the_held_shape(tmp_path):
    ...
    system, _path = build_system(4, radius=0.9, out_dir=tmp_path)
    ...
    assert achieved[1] - achieved[0] > 0.4
...
def test_cbirrt_plans_a_path_on_the_array(tmp_path, arm_count):
    ...
    result = planner.plan(start=start, goal_tsrs=[region], seed=1, return_details=True)
    assert result.success, f"plan failed: {result.failure_reason}"

    # A usable path: every waypoint in limits, and the end inside the region.
